...
exports.build_shows_from_recordings = () => {
	...
	return new Promise(async function(resolve, reject) {
		...
		const q_set = (await models.pool.query("SET FOREIGN_KEY_CHECKS=0;"));
		const q_empty = (await models.pool.query("TRUNCATE TABLE Shows;"));
		const q_reset = (await models.pool.query("SET FOREIGN_KEY_CHECKS=1;"));
		
		const dates = models.pool.query(
			'SELECT ' +
