...
const refreshData = function(done) {
	...
	return request(SEARCH_URL(), function(err, response, body) {
		...
		logger.log('info', 'got search results');
		
		const recordings = body['response']['docs'];
		
		logger.log('info', recordings.length, 'recordings');
		...
		return async.mapLimit(recordings, 1, function(small_show, cb) {
			logger.log('info', 'requesting ' + small_show.date);
			return loadShow(small_show, cb);
		}, function(err) {
			...
			return cache_year_stats(function(err) {
				...
				logger.log('info', "reweighing averages");
				return refresh_weighted_avg(done);
			});
		});
	});
};
...
const refreshShow = function(artist, id, done) {
	logger.log('info', 'requesting search url');
	...
	return request(SINGLE_URL(id), function(err, httpres, body) {
		...
		logger.log('info', `requesting ${(show.metadata.indentifier != null ? show.metadata.indentifier[0] : undefined)}`);
		
		return loadShow({identifier: show.metadata.identifier[0]}, done);
	});
};
...
var refresh_weighted_avg = (artist, done) => {
	...
	.catch(done)
	.then(function(dates) {
		logger.log('info', 'mapping dates: ' + dates.length);
		...
		return dates.map(date => {
			...
			.then(function(tapes) {
				...
				logger.log('info', 'updating ' + proms.length + ' tapes');
...
var cache_year_stats = function(done) {
	logger.log('info', "Caching year information");
	...
	.then(() => {
		...
		.then(() => {
			...
			.catch(done)
			.then(function() {
				logger.log('info', "Complete year cache");
				return done();
			});
		});
	});
...
var loadShow = (small_show, cb) => {
	...
	.then(function(pre_existing_show) {
		...
		if (pre_existing_show !== null) {
			logger.log('info', "this archive identifier is already in the db");
			return cb();
		}
		...
		return request(single_url, function(err, httpres, body) {
			logger.log('info', 'GET ' + single_url);
			...
			if (!body.metadata.date) {
				logger.log('info', 'no date:', body.metadata.title);
			...
			const showCreated = function(show, created) {
				...
				if (!created) {
					logger.log('info', "this show is already in the db; ensuring archive_collection tracks are present");
					return cb();
				} else {
					logger.log('info', "show created! looking for venue");
				}
				...
				.catch(cb)
				.spread(function(venue) {
					logger.log('info', "building tracks");
					logger.log('info', "setting venue and tracks");
			...
			logger.log('info', "looking for show in db");
