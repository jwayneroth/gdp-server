...
router.post('/lists', ensureAuth(['*']), async (req, res, next) => {
	...
	if (media && media.type && media.id) {
		const update = await api.addListMedia(list, media);
	}
...
router.get('/lists/public', ensureAuth(['*']), async (req, res, next) => {
	const lists = await models.List.findAll({
		where: { isPublic: true },
		//order: ['category', 'DESC'],
	});
	if (!lists) return next(createError(500, 'we experienced an unknown error. Please try again later.'));
...
router.put('/lists/:id', ensureAuth(['*']), async (req, res, next) => {
	...
	const {media, title, isPublic} = req.body;
	
	if (media && media.hasOwnProperty('type') && media.hasOwnProperty('id')) {
		if (media.hasOwnProperty('delete')) {
			console.log('DELETING ' + media.id);
			update = await api.removeListMedia(list, media);
		} else {
			update = await api.addListMedia(list, media);
		}
	}
