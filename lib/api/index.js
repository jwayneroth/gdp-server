...
exports.teasers_by_ids = async function(media_type, ids) {
	
	const q_where = {id: ids}, q_order = [];
	
	let q_model, q_attributes = ['title'], q_include = [];
	
	if (media_type === 'track') {
		q_model = models.Track;
		q_attributes.push('id', 'length', 'file');
		q_include.push({
			model: models.Recording,
		...
		q_model = models.Recording;
		q_attributes.push('ShowId', 'date');
		q_order.push(['date', 'ASC']);
	
	} else if (media_type === 'show') {
		q_model = models.Show;
		q_attributes.push(['id', 'ShowId'], 'date');
		q_order.push(['date', 'ASC']);
	...
	const teasers = (await q_model.findAll({
		where: q_where,
	...
	// flatten tracks
	if (media_type === 'track') {
		return teasers.map(track => {
			const {id, title, length, file, Recording} = track;
			return {
				id,
				title,
				length,
...
exports.modelByMediaType = function(type) {
	if (type === 'show') {
		return models.Show
...
exports.addListMedia = function(list, {type, id}) {
	switch(type) {
		case 'show':
			return list.addShow([id]);
		case 'recording':
			return list.addRecordings([id]);
		case 'track':
			return list.addTracks([id]);
	}
	return false;
}

exports.removeListMedia = function(list, {type, id}) {
	switch(type) {
		case 'show':
			return list.removeShow([id]);
		case 'recording':
			return list.removeRecordings([id]);
		case 'track':
			return list.removeTracks([id]);
	}
	return false;
}
